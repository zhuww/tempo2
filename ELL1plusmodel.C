...
double ELL1plusmodel(pulsar *psr,int p,int ipos,int param)
{
   double an,x0,m2,orbits,phase,e1,e2,dre,drep,drepp,brace,dlogbr,ds,da,pb;
  double eps1,eps2,eps1dot,eps2dot,si,a0,b0;
  double d2bar,torb,Csigma,Cx,Ceps1,Ceps2,Cm2,Csi,pbdot,xpbdot,x,xdot,am2,diff;
  long double ct, t0asc, tt0, tt;
  int norbits;
  double SUNMASS = 4.925490947e-6;
  const char *CVS_verNum = "$Revision: 1.6 $";

  if (displayCVSversion == 1) CVSdisplayVersion("ELL1plusmodel.C","ELL1plusmodel()",CVS_verNum);
...
do { 
  ...
  delta = dre + ds + da;
  //printf("dre: %f ; ds: %f; da: %f;", dre, ds, da);
