...
static double calcDHp( double ae, double h3, double h4, int nharm, int sel);
long double getParameterp(pulsar *psr,int p,int k);

void calcGRp(double mtot,double m2,double x,double ecc,double an,double afac,
            double f0, double *dr,double *dth,double *er,double *eth,
            double *xk,double *si,double *gamma, double *pbdot,double *a0,
            double *b0);
void getKeplerianp(pulsar *psr,int com,double *pb,long double *t0,double *ecc,
                  double *omz,double *x,double *eps1,double *eps2,
                  long double *t0asc,double *shapmax,double *kom,double *kin);
void addKeplerianJumpsp(pulsar *psr,int ipos,double *torb,double *x,double *ecc,
                       double *omz,double *pb);
void getPostKeplerianp(pulsar *psr,int com,double an,double *si,double *m2,
...
void updateParametersp(double edot,double xdot,double eps1dot,double eps2dot,
                      long double tt0,double *ecc,double *x,double *eps1,
                      double *eps2);
void deriveKeplerianp(double pb,double kom,double *an,double *sin_omega,
                     double *cos_omega);
void derivePostKeplerianp(double mtot,double m2,double dr,double dth,
                         double ecc,double *m1,double *er,double *eth);
void KopeikinTermsp(pulsar *psr,int ipos,double ki,double pmra,double sin_omega,
...
void computeUp(double phase,double ecc,double *u);

double T2plusmodel(pulsar *psr,int p,int ipos,int param,int arr)
{
  double an;
  double pb,omdot;
  ...
  double e1,e2,eps1,eps2,eps1dot,eps2dot;
  double ceps1,ceps2;
  ...
  for (com = com1; com < com2;com++)
    {      
      ...
      getKeplerianp(&psr[p],com,&pb,&t0,&ecc,&omz,&x,&eps1,&eps2,&t0asc,
                   &shapmax,&kom,&ki);
      /* Now add in the jumps */
      addKeplerianJumpsp(&psr[p],ipos,&torb,&x,&ecc,&omz,&pb);
      /* Parameters derived from the Keplerian parameters */
      deriveKeplerianp(pb,kom,&an,&sin_omega,&cos_omega);
      /* Obtain post-Keplerian parameters */
      getPostKeplerianp(&psr[p],com,an,&si,&m2,&mtot,&omdot,&gamma,&xdot,&xpbdot,
      ...
      if (psr[p].param[param_mtot].paramSet[com]==1)
        {
          calcGRp(mtot,m2,x,ecc,an,afac,(double)psr[p].param[param_f].val[0],
                 &dr,&dth,&er,&eth,&xk,&si,&gamma,&pbdot,&a0,&b0);
          omdot   = xomdot + xk; 
        }
      /* Derive parameters from the post-Keplerian parameters */
      derivePostKeplerianp(mtot,m2,dr,dth,ecc,&m1,&er,&eth);
      /* Obtain delta T */
      ...
      //printf("ct: %.15Lf ", ct);
      //printf("t0asc: %.15Lf ", t0asc);
      //printf("tt0: %.15Lf \n", tt0);

      updateParametersp(edot,xdot,eps1dot,eps2dot,tt0,&ecc,&x,&eps1,&eps2);

      double const epsNum = 1.e-12; // inversion of timing model by iteration: begin of loop
      double tt,diff,x0;
      double delta = 0.0;
    ...
    do { 
      ...
      //printf("EPS1DOT, EPS2DOT: %g %g", eps1dot, eps2dot); 
	  ...
	  //          logdbg("going to Kopeikin");
          ...
          if (psr[p].param[param_kin].paramSet[com]==1 && 
            ...
            {
	      //              logdbg("going to do KopeikinTermsp");
              KopeikinTermsp(&psr[p],ipos,ki,pmra,sin_omega,pmdec,cos_omega,tt,
                            dpara,daop,si,&x,&DK011,&DK012,&DK021,&DK022,
                            &DK031,&DK032,&DK041,&DK042,&DK013,&DK014,&DK023,
                            &DK024,&DK033,&DK034,&DK043,&DK044);
	      //              logdbg("did KopeikinTermsp");
	  ...
	  ds=-2*m2*dlogbr;        /* Equation 26 */
      ...
      else if (psr[p].param[param_eps1].paramSet[com]==1)  /* ELL1 model */
        {
          ...
          //e1 = eps1+eps1dot*tt;
          //e2 = eps2+eps2dot*tt;
          e1 = eps1; //updateParametersp does the time derivative update
          e2 = eps2;
          //dre  = x*(sin(phase)-0.5*(eps1*cos(2.0*phase)-eps2*sin(2.0*phase)));
          dre  = x*(sin(phase)-0.5*(e1*cos(2.0*phase)-e2*sin(2.0*phase))) -
              (x*(-2.*e1*e2*cos(phase) + 6.*e1*e2*cos(3.*phase) + 
                  3.*pow(e1,2)*sin(phase) + 5.*pow(e2,2)*sin(phase) +
                  3.*pow(e1,2)*sin(3.*phase) - 3.*pow(e2,2)*sin(3.*phase)))/8;
          //drep = x*cos(phase);
          //drepp=-x*sin(phase);
	  //          logdbg("going to Kopeikin");
          /* Update parameters due to proper motion - Kopeikin 1996 */
	  ...
	  if ( psr[p].param[param_h3].paramSet[0] * psr[p].param[param_stig].paramSet[0] == 1 
	    ...
	    if( mode == 0 ){
	      ...
	      ds = calcDHp( TrueAnom, h3, h4, nharm, 0 );
	    }
  ...
  //delta = dre + allTerms(ds+da+DAOP+DSR+DOP); //allTerms was set to 0, now I
  //want it to be 1. Why ignore all these terms? -- Weiwei Zhu
 
  //printf("delta_old and new: %g %g", delta_old, delta);
  //printf("diff: %g\n", diff);
}while( diff > epsNum);
      ...
      if (param==-1 && com == psr[p].nCompanion-1) return torb;
      else if (param!=-1 && com==arr)
        {
          ...
          if (param==param_pb)	         return -csigma*an*SECDAY*tt/pb; 
          else if (param==param_a1)      return cx;
          else if (param==param_ecc)     return ce;
          else if (param==param_edot)     return ce*tt;
            ...
            if(psr[p].param[param_pbdot].nLinkFrom>0){
              return 0.5*tt*(-csigma*an*SECDAY*tt/(pb*SECDAY));
            ...
            else  return 0.5*tt*(-csigma*an*SECDAY*tt/(pb*SECDAY));
          }
          else if (param==param_sini)    return csi;
          else if (param==param_gamma)   return cgamma;
          else if (param==param_m2)      return cm2*SUNMASS;
          else if (param==param_a1dot)   return cx*tt;
          else if (param==param_eps1)    return ceps1;
          else if (param==param_eps1dot) return ceps1*tt;
          else if (param==param_eps2dot) return ceps2*tt;
	    ...
	    if( mode == 0 || mode == 2)
	      ...
	      return( calcDHp( TrueAnom, h3, h4, nharm, 3 ) );
	    else{
	    ...
	    if( mode == 2 )
	      ...
	      return( calcDHp( TrueAnom, h3, h4, nharm, 4 ) );
	  }	  
          ...
          if (psr[p].param[param_bpjep].paramSet[arr]==1 && 
              psr[p].obsn[ipos].bbat > psr[p].param[param_bpjep].val[arr])
            {
              if (param==param_bpjph)
                ...
                return -csigma*an*SECDAY*tt/(pb*SECDAY); 
            }
          else
            return 0.0;
        }
...
void updateT2plus(pulsar *psr,double val,double err,int pos,int arr){
  ...
  else if( pos==param_tasc) {
    psr->param[pos].val[arr] += (long double) val;
    psr->param[pos].err[arr]  = (long double) err;
    //printf("what is tasc like in update: %.15LF \n", psr->param[pos].val[arr] );
  } 
...
void getKeplerianp(pulsar *psr,int com,double *pb,long double *t0,double *ecc,
                  ...
                  long double *t0asc,double *shapmax,double *kom,double *kin)
{
  *pb  = getParameterp(psr,param_pb,com)*SECDAY;
  *t0  = getParameterp(psr,param_t0,com);
  *ecc = getParameterp(psr,param_ecc,com); 
  *omz = getParameterp(psr,param_om,com);
  *x   = getParameterp(psr,param_a1,com);
  
  *eps1= getParameterp(psr,param_eps1,com);
  *eps2= getParameterp(psr,param_eps2,com);
  *t0asc = getParameterp(psr,param_tasc,com);
  //printf("what's t0asc when readed out: %.15Lf \n", *t0asc);
  
  *shapmax = getParameterp(psr,param_shapmax,com);

  *kom = getParameterp(psr,param_kom,com)*M_PI/180.0;
  *kin = getParameterp(psr,param_kin,com)*M_PI/180.0;
}
...
void getPostKeplerianp(pulsar *psr,int com,double an,double *si,double *m2,
  ...
  *si      = getParameterp(psr,param_sini,com);
  if (*si > 1.0)
    {
  ...
  *m2      = getParameterp(psr,param_m2,com)*SUNMASS;
  *mtot    = getParameterp(psr,param_mtot,com)*SUNMASS;
  *omdot   = getParameterp(psr,param_omdot,com)/(rad2deg*365.25*SECDAY*an);
  *gamma   = getParameterp(psr,param_gamma,com);
  *xdot    = getParameterp(psr,param_a1dot,com);
  *xpbdot  = getParameterp(psr,param_xpbdot,com);
  *pbdot   = getParameterp(psr,param_pbdot,com);
  *edot    = getParameterp(psr,param_edot,com);
  *pmra    = getParameterp(psr,param_pmra,com)
    * M_PI/(180.0*3600.0e3)/(365.25*86400.0);
  *pmdec   = getParameterp(psr,param_pmdec,com)
    * M_PI/(180.0*3600.0e3)/(365.25*86400.0);
  *dpara   = getParameterp(psr,param_px,com)*pxConv;
  *dr      = getParameterp(psr,param_dr,com);
  *dth     = getParameterp(psr,param_dth,com);
  *a0      = getParameterp(psr,param_a0,com);
  *b0      = getParameterp(psr,param_b0,com);
  *xomdot  = getParameterp(psr,param_xomdot,com)/(an*rad2deg*365.25*86400.0);
  *afac    = getParameterp(psr,param_afac,com);
  *eps1dot = getParameterp(psr,param_eps1dot,com);
  *eps2dot = getParameterp(psr,param_eps2dot,com);
  *daop    = getParameterp(psr,param_daop,com)*1e-3/pxConv;
}
