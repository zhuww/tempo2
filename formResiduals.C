	   ...
	   if (psr[p].param[param_pb].paramSet[0]==1 || psr[p].param[param_fb].paramSet[0]==1) 
	     {
	       ...
	       else if (strcmp(psr[p].binaryModel,"T2+")==0)    torb = T2plusmodel(psr,p,i,-1,0);
	       ...
	       else if( strcmp( psr[p].binaryModel, "ELL1+" ) == 0) torb = ELL1plusmodel( psr, p, i, -1 );
...
//		   else  
	...
	//	       psr[p].gwm_phi = lambda + 1.5708;
       ...
       for (i=0;i<psr[p].nobs;i++)
	 {
       ...
       //for (i=0;i<psr[p].nobs;i++)
         //if( isnan(psr[p].obsn[i].residual)) printf("i %d\n", i);
